...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.n_pred_var = configs.enc_in - (configs.exo + configs.exo_future) # assume that the rain(exo) index is -3 and the future rain(exo_future) index is -2
        ...
        self.decoder = nn.Linear(d_model*conv_out_len, target_window*self.n_pred_var) 
    ...
    def forward(self,x): # x: [Batch, Input length, nvars]
        ...
        x = self.decoder(x)   # -> [Batch, Output Length*n_pred_var]
        #  reshape to [Batch, Output Length, n_pred_var]
        x = x.view(x.size(0), -1, self.n_pred_var) # -> [Batch, Output Length, n_pred_var]
        return x
